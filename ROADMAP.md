...
| `wasp1` top | PASS | Full hierarchy lint, reset-default smoke, SoC JTAG debug smoke, remote-bitbang socket smoke, OpenOCD/GDB process smoke, generated OTP firmware boot-to-UART smoke, DMA real-memory-copy firmware smoke, timer IRQ firmware smoke, OTP programming-register firmware smoke, debug status, and idle IO stability pass |
...
| `llvm_s1` | PASS | Stage-1 BSP, Homebrew LLVM/lld strict RV32I compile/link smoke, objcopy, OTP image utility, generated UART/DMA/timer-IRQ/OTP-programming firmware images, local sparse LLVM source checkout, and wasp1 OTP boot/DMA/timer/programming smokes pass |
...
| full system software | TODO | Machine timer interrupt firmware smoke passes; external interrupt firmware and longer generated-image regressions remain |

## Near-Term Plan

```text
1. Add external interrupt firmware software tests through INTC
