...
def render_debug_dmi_regs_l3() -> None:
    """Render detailed DMI register-transfer timing and priority diagram."""
    canvas = Canvas(2200, 1320)
    canvas.grid()
    canvas.text(40, 34, "DEBUG_DMI_REGS L3 REGISTER TRANSFER", scale=3, color=BLACK)
    canvas.box_text(
        40,
        90,
        [
            "BLUE IF     : EXTERNAL CONNECTION",
            "YELLOW COMB : STATE FREE DECODE OR MUX",
            "GREEN SEQ   : CLK_I WITH ASYNC RST_NI",
            "ONE RESPONSE SLOT",
            "ONE IMPLEMENTED HART: HARTSEL ZERO",
        ],
        "LEGEND",
    )

    canvas.state(70, 390, 220, 105, ("IF", "DTM", "REQUEST"), BLUE)
    canvas.state(390, 360, 280, 125, ("COMB", "OP ADDR", "DECODE"), YELLOW)
    canvas.state(800, 185, 300, 125, ("COMB", "READ DATA", "STATUS MUX"), YELLOW)
    canvas.state(810, 430, 280, 125, ("SEQ", "RESPONSE", "SLOT"), GREEN)
    canvas.state(1240, 430, 250, 105, ("IF", "DTM", "RESPONSE"), BLUE)

    canvas.state(380, 750, 300, 125, ("SEQ", "DMCONTROL", "REGISTERS"), GREEN)
    canvas.state(805, 750, 310, 125, ("COMB", "HART SELECT", "OUTPUT GATE"), YELLOW)
    canvas.state(1250, 750, 260, 125, ("IF", "HART", "CONTROL"), BLUE)

    canvas.state(380, 1030, 300, 125, ("SEQ", "ABSTRACT", "REGISTERS"), GREEN)
    canvas.state(815, 1030, 290, 125, ("COMB", "ABSTRACTCS", "IMAGE"), YELLOW)
    canvas.state(1260, 1030, 270, 125, ("IF", "COMMAND", "EXECUTOR"), BLUE)
    canvas.state(1690, 750, 280, 125, ("COMB", "DMSTATUS", "IMAGE"), YELLOW)
    canvas.state(1690, 1030, 280, 125, ("IF", "HART", "STATUS"), BLUE)

    canvas.path_arrow([(290, 442), (390, 422)], "REQ FIRE")
    canvas.path_arrow([(670, 400), (810, 465)], "RESP CLASS")
    canvas.path_arrow([(1100, 247), (1160, 247), (1160, 465), (1090, 465)], "READ DATA")
    canvas.path_arrow([(1090, 492), (1240, 482)], "VALID RESP DATA")
    canvas.path_arrow([(680, 750), (805, 812)], "HARTSEL LEVELS")
    canvas.path_arrow([(1115, 812), (1250, 812)], "HALT RESUME RESET")
    canvas.path_arrow([(680, 1092), (815, 1092)], "BUSY CMDERR")
    canvas.path_arrow([(1105, 1092), (1260, 1092)], "COMMAND DATA0")
    canvas.path_arrow([(1970, 1092), (2035, 1092), (2035, 812), (1970, 812)], "RUN HALT ACK RESET")
    canvas.path_arrow([(1690, 812), (1580, 812), (1580, 247), (1100, 247)], "DMSTATUS")
    canvas.path_arrow([(815, 1092), (750, 1092), (750, 247), (800, 247)], "ABSTRACTCS")
    ...
    canvas.box_text(
        ...
        [
            "&& REQ_VALID",
            "&& REQ_READY",
            "ACTION RSP_VALID_Q=1",
            "ACTION SAMPLE RESP STATUS AND DATA",
            "FAILED FOR UNKNOWN ADDR OR OP=3",
        ],
        "REQUEST ACCEPT",
    )
    canvas.box_text(
        ...
        165,
        [
            "REQ_READY = !RSP_VALID_Q OR RSP_READY",
            "HOLD RESP FIELDS WHILE !RSP_READY",
            "CONSUME AND REPLACE MAY SHARE ONE EDGE",
        ],
    ...
    canvas.box_text(
        ...
        610,
        [
            "PRIORITY 1: ASYNC RST_NI CLEARS ALL",
            "PRIORITY 2: DMCONTROL DMACTIVE=0 CLEARS DM STATE",
            "PRIORITY 3: ACCEPTED DMCONTROL WRITE UPDATES FIELDS",
            "PRIORITY 4: RESUMEACK CLEARS HELD RESUMEREQ",
            "ACTIVATION WRITE CHANGES DMACTIVE ONLY",
        ],
        "CONTROL UPDATE",
    )
    canvas.box_text(
        ...
        930,
        [
            "COMMAND WRITE && !BUSY: CAPTURE AND PULSE VALID",
            "COMMAND OR DATA0 WRITE && BUSY: IGNORE AND SET CMDERR=1",
            "EXECUTOR DATA0_WE: UPDATE DATA0",
            "EXECUTOR ERROR: SET FIRST NONZERO CMDERR",
            "ABSTRACTCS W1C: CLEAR SELECTED CMDERR BITS",
        ],
        "ABSTRACT UPDATE",
    )
    canvas.box_text(
        ...
        560,
        [
            "HARTSEL=0: ANY FIELDS EQUAL ALL FIELDS",
            "HARTSEL!=0: ANYNONEXISTENT=ALLNONEXISTENT=1",
            "AUTHENTICATED=1",
            "VERSION=2 FOR DEBUG SPEC V0.13",
        ],
        "STATUS RULES",
    )
    canvas.action_label(720, 585, ["SEPARATE SEQ BANKS", "NO COMBINED SEQ+COMB BLOCK"])
    canvas.text(40, 1275, "NOTE: GENERATED BY DOCS/TOOLS/RENDER_STATE_PNGS.PY", scale=2, color=DARK_GRAY)
    canvas.write_png(Path("debug/docs/images/debug_dmi_regs_state.png"))


def render_debug_halt_ctrl_l3() -> None:
    """Render the single-hart halt/resume FSM and sticky status registers."""
    canvas = Canvas(2200, 1320)
    canvas.grid()
    canvas.text(40, 34, "DEBUG_HALT_CTRL L3 HALT RESUME FSM", scale=3, color=BLACK)
    canvas.box_text(
        40,
        90,
        [
            "BLUE IF     : EXTERNAL CONNECTION",
            "YELLOW COMB : STATE FREE LOGIC",
            "GREEN SEQ   : CLK_I WITH ASYNC RST_NI",
            "HALT HAS PRIORITY OVER RESUME",
            "CORE OWNS RUNNING AND HALTED STATUS",
        ],
        "LEGEND",
    )

    canvas.state(100, 405, 250, 110, ("IF", "DM CONTROL", "INTENT"), BLUE)
    canvas.state(470, 405, 280, 110, ("COMB", "NEXT STATE", "LOGIC"), YELLOW)
    canvas.state(930, 195, 220, 105, ("SEQ", "IDLE"), GREEN)
    canvas.state(930, 425, 240, 105, ("SEQ", "HALT WAIT"), GREEN)
    canvas.state(930, 675, 260, 105, ("SEQ", "RESUME WAIT"), GREEN)
    canvas.state(1370, 425, 290, 115, ("COMB", "REQUEST", "DECODE"), YELLOW)
    canvas.state(1810, 425, 240, 115, ("IF", "CORE", "REQUEST"), BLUE)
    canvas.state(470, 930, 290, 115, ("SEQ", "RESUMEACK", "REGISTER"), GREEN)
    canvas.state(930, 930, 290, 115, ("SEQ", "HAVERESET", "REGISTER"), GREEN)
    canvas.state(1390, 930, 270, 115, ("COMB", "STATUS", "ROUTE"), YELLOW)
    canvas.state(1810, 930, 240, 115, ("IF", "DMSTATUS"), BLUE)
    canvas.state(1390, 705, 270, 115, ("IF", "CORE", "STATUS"), BLUE)

    canvas.path_arrow([(350, 460), (470, 460)], "ACTIVE HALT RESUME")
    canvas.path_arrow([(750, 430), (820, 350), (820, 247), (930, 247)])
    canvas.path_arrow([(1170, 477), (1370, 477)], "HALT WAIT")
    canvas.path_arrow([(1190, 727), (1280, 727), (1280, 510), (1370, 510)], "RESUME WAIT")
    canvas.path_arrow([(1660, 482), (1810, 482)], "HALT REQ RESUME REQ")
    canvas.path_arrow([(1660, 762), (1730, 762), (1730, 460), (1660, 460)], "RUN HALT")
    canvas.path_arrow([(1660, 762), (1735, 762), (1735, 987), (1660, 987)], "STATUS")
    canvas.path_arrow([(760, 987), (1390, 987)], "RESUME ACK")
    canvas.path_arrow([(1220, 987), (1390, 987)], "HAVE RESET")
    canvas.path_arrow([(1660, 987), (1810, 987)], "ANY ALL STATUS")

    canvas.path_arrow([(1040, 300), (1040, 425)], "START HALT")
    canvas.path_arrow([(1170, 455), (1260, 455), (1260, 247), (1150, 247)], "DONE OR CANCEL")
    canvas.path_arrow([(1100, 300), (1330, 350), (1330, 727), (1190, 727)], "START RESUME")
    canvas.path_arrow([(930, 727), (760, 727), (760, 247), (930, 247)], "R DONE")
    canvas.path_arrow([(1190, 700), (1350, 620), (1350, 477), (1170, 477)], "HALT WINS")
    ...
    canvas.box_text(
        ...
        [
            ...
            "&& HALTREQ_I",
            "&& !CORE_HALTED_I",
            "ACTION ASSERT HALT REQUEST IN WAIT STATE",
        ],
        "HALT START",
    )
    canvas.box_text(
    ...
    canvas.box_text(
        ...
        1080,
        [
            "NEW RESUME: CLEAR OLD ACK",
            "CORE RUNNING: SET ACK",
            "HOLD ACK THROUGH IDLE FOR DMI POLLING",
            "DMACTIVE=0 OR HART RESET: CLEAR ACK",
        ],
        "RESUMEACK PRIORITY",
    )
    canvas.box_text(
        ...
        1080,
        [
            "ASYNC RST_NI: SET HAVERESET",
            "HART_RESET_EVENT_I: SET AND ABORT FSM",
            "ACTIVE ACKHAVERESET_I: CLEAR",
            "RESET EVENT WINS OVER ACK",
        ],
        "HAVERESET PRIORITY",
    )
    canvas.box_text(
        ...
        140,
        [
            "DMACTIVE=0: STATE D=IDLE",
            "HART RESET: STATE Q=IDLE",
            "REQUEST OUTPUTS GATED LOW IMMEDIATELY",
            "RESUME WAIT IGNORES EARLY RESUMEREQ CLEAR",
        ],
        "ABORT AND HOLD RULES",
    )
    canvas.action_label(1330, 570, ["HALT REQ = ACTIVE AND HALT WAIT", "RESUME REQ = ACTIVE AND RESUME WAIT"])
    canvas.text(40, 1275, "NOTE: GENERATED BY DOCS/TOOLS/RENDER_STATE_PNGS.PY", scale=2, color=DARK_GRAY)
    canvas.write_png(Path("debug/docs/images/debug_halt_ctrl_fsm.png"))


def render_debug_reg_access_l3() -> None:
    """Render detailed GPR request/response and stale-response drain behavior."""
    canvas = Canvas(2400, 1380)
    canvas.grid()
    canvas.text(40, 34, "DEBUG_REG_ACCESS L3 GPR TRANSACTION FSM", scale=3, color=BLACK)
    canvas.box_text(
        40,
        90,
        [
            "BLUE IF     : EXTERNAL CONNECTION",
            "YELLOW COMB : STATE FREE HANDSHAKE DRIVE",
            "GREEN SEQ   : CLK_I WITH ASYNC RST_NI",
            "ONE COMMAND OUTSTANDING",
            "DROP RSP DRAINS A FLUSHED CORE TRANSACTION",
        ],
        "LEGEND",
    )

    # Main state lane keeps normal progress left-to-right and cancellation below.
    canvas.state(70, 350, 230, 105, ("IF", "UPSTREAM", "COMMAND"), BLUE)
    canvas.state(390, 350, 190, 105, ("SEQ", "IDLE"), GREEN)
    canvas.state(710, 350, 230, 105, ("SEQ", "CORE REQ"), GREEN)
    canvas.state(1080, 350, 250, 105, ("SEQ", "CORE WAIT"), GREEN)
    canvas.state(1480, 350, 250, 105, ("SEQ", "LOCAL RSP"), GREEN)
    canvas.state(1900, 350, 230, 105, ("IF", "UPSTREAM", "RESPONSE"), BLUE)
    canvas.state(1090, 650, 250, 105, ("SEQ", "DROP RSP"), GREEN)
    canvas.state(710, 650, 240, 105, ("IF", "CORE GPR", "CHANNEL"), BLUE)

    canvas.path_arrow([(300, 402), (390, 402)], "CMD VALID READY")
    canvas.path_arrow([(580, 402), (710, 402)], "CMD FIRE")
    canvas.path_arrow([(940, 402), (1080, 402)], "REQ FIRE")
    canvas.path_arrow([(1330, 402), (1480, 402)], "RSP FIRE")
    canvas.path_arrow([(1730, 402), (1900, 402)], "RSP VALID READY")
    canvas.path_arrow([(1480, 385), (1410, 300), (825, 300), (825, 350)], "SAME CYCLE RSP")
    canvas.path_arrow([(1205, 455), (1205, 650)], "FLUSH NO RSP")
    canvas.path_arrow([(1090, 702), (980, 702), (980, 505), (485, 505), (485, 455)], "STALE RSP")
    canvas.path_arrow([(825, 650), (825, 455)], "REQ RSP")

    # Datapath register lane is deliberately separate from the FSM states.
    canvas.state(220, 950, 300, 115, ("SEQ", "COMMAND", "REGISTERS"), GREEN)
    canvas.state(650, 950, 300, 115, ("COMB", "CORE REQUEST", "DRIVE"), YELLOW)
    canvas.state(1080, 950, 270, 115, ("IF", "DEBUG IF", "DM GPR"), BLUE)
    canvas.state(1480, 950, 300, 115, ("SEQ", "RESPONSE", "REGISTERS"), GREEN)
    canvas.state(1920, 950, 300, 115, ("COMB", "UPSTREAM", "DRIVE"), YELLOW)

    canvas.path_arrow([(520, 1007), (650, 1007)], "WRITE ADDR WDATA")
    canvas.path_arrow([(950, 1007), (1080, 1007)], "REQ VALID FIELDS")
    canvas.path_arrow([(1350, 1007), (1480, 1007)], "RSP DATA ERROR")
    canvas.path_arrow([(1780, 1007), (1920, 1007)], "STABLE RESULT")
    ...
    canvas.box_text(
        ...
        [
            "CORE REQ + FLUSH",
            "ACTION GATE REQ VALID LOW",
            "ACTION STATE D = IDLE",
            "NOTE CORE NEVER ACCEPTS THIS REQUEST",
        ],
        "FLUSH BEFORE ACCEPT",
    )
    canvas.box_text(
        ...
        560,
        [
            "CORE WAIT + FLUSH + !RSP FIRE",
            "ACTION STATE D = DROP RSP",
            "ACTION KEEP CORE RSP READY HIGH",
            "ACTION BLOCK NEW COMMANDS",
        ],
        "FLUSH AFTER ACCEPT",
    )
    canvas.box_text(
        ...
        660,
        [
            "DROP RSP + CORE RSP FIRE",
            "ACTION CONSUME WITHOUT CAPTURE",
            "ACTION STATE D = IDLE",
            "ACTION NEVER ASSERT LOCAL RSP VALID",
        ],
        "STALE RESPONSE DRAIN",
    )
    canvas.box_text(
        40,
        1130,
        [
            "CMD FIRE: CAPTURE WRITE ADDR WDATA",
            "CORE RSP FIRE && !FLUSH: CAPTURE DATA ERROR",
            "LOCAL RSP HOLDS UNTIL RSP READY OR FLUSH",
            "RESET: STATE IDLE AND ALL DATA REGISTERS ZERO",
        ],
    ...
    canvas.box_text(
        ...
        1130,
        [
            "CORE REQ FIELDS COME ONLY FROM COMMAND REGS",
            "CORE RSP READY IN WAIT DROP OR SAME CYCLE ACCEPT",
            "UPSTREAM RESULT COMES ONLY FROM RESPONSE REGS",
            "NO COMBINATIONAL CORE DATA TO UPSTREAM PATH",
        ],
        "STABILITY GUARANTEE",
    )
    canvas.text(40, 1335, "NOTE: GENERATED BY DOCS/TOOLS/RENDER_STATE_PNGS.PY", scale=2, color=DARK_GRAY)
    canvas.write_png(Path("debug/docs/images/debug_reg_access_fsm.png"))


def render_debug_abstract_cmd_l3() -> None:
    """Render Access Register decode, execution, and completion side effects."""
    canvas = Canvas(2400, 1380)
    canvas.grid()
    canvas.text(40, 34, "DEBUG_ABSTRACT_CMD L3 ACCESS REGISTER FSM", scale=3, color=BLACK)
    canvas.box_text(
        40,
        90,
        [
            "BLUE IF     : EXTERNAL CONNECTION",
            "YELLOW COMB : FIELD DECODE OR OUTPUT MUX",
            "GREEN SEQ   : CLK_I WITH ASYNC RST_NI",
            "SUPPORTED: RV32 GPR X0 THROUGH X31",
            "ERRORS: NOTSUP HALT_RESUME EXCEPTION",
        ],
        "LEGEND",
    )

    # Command/control state lane.
    canvas.state(50, 350, 230, 105, ("IF", "DMI COMMAND", "DATA0"), BLUE)
    canvas.state(350, 350, 250, 105, ("COMB", "FIELD", "DECODER"), YELLOW)
    canvas.state(700, 350, 180, 105, ("SEQ", "IDLE"), GREEN)
    canvas.state(1010, 350, 200, 105, ("SEQ", "ISSUE"), GREEN)
    canvas.state(1340, 350, 190, 105, ("SEQ", "WAIT"), GREEN)
    canvas.state(1670, 350, 240, 105, ("SEQ", "COMPLETE"), GREEN)
    canvas.state(2100, 350, 230, 105, ("IF", "DMI REGS", "RESULT"), BLUE)
    canvas.state(1170, 650, 260, 105, ("IF", "REG ACCESS", "CMD RSP"), BLUE)

    canvas.path_arrow([(280, 402), (350, 402)], "RAW COMMAND")
    canvas.path_arrow([(600, 402), (700, 402)], "DECODE CLASS")
    canvas.path_arrow([(880, 402), (1010, 402)], "VALID TRANSFER")
    canvas.path_arrow([(1210, 402), (1340, 402)], "CMD FIRE")
    canvas.path_arrow([(1530, 402), (1670, 402)], "RSP FIRE")
    canvas.path_arrow([(1910, 402), (2100, 402)], "CMDERR OR DATA0")
    canvas.path_arrow([(790, 350), (790, 260), (1790, 260), (1790, 350)], "ERROR OR NOOP")
    canvas.path_arrow([(1110, 455), (1220, 650)], "REG COMMAND")
    canvas.path_arrow([(1330, 650), (1435, 455)], "REG RESPONSE")
    canvas.path_arrow([(1340, 385), (1280, 290), (1110, 290), (1110, 350)], "HART LOST")

    # Datapath lane separates storage and combinational drive.
    canvas.state(180, 950, 300, 115, ("SEQ", "REQUEST", "REGISTERS"), GREEN)
    canvas.state(610, 950, 290, 115, ("COMB", "REG COMMAND", "DRIVE"), YELLOW)
    canvas.state(1030, 950, 270, 115, ("IF", "REG ACCESS", "CHANNEL"), BLUE)
    canvas.state(1430, 950, 300, 115, ("SEQ", "COMPLETION", "REGISTERS"), GREEN)
    canvas.state(1870, 950, 310, 115, ("COMB", "DATA0 CMDERR", "PULSE DRIVE"), YELLOW)

    canvas.path_arrow([(480, 1007), (610, 1007)], "WRITE ADDR WDATA")
    canvas.path_arrow([(900, 1007), (1030, 1007)], "VALID READY")
    canvas.path_arrow([(1300, 1007), (1430, 1007)], "DATA ERROR")
    canvas.path_arrow([(1730, 1007), (1870, 1007)], "ERROR READ VALID")
    ...
    canvas.box_text(
        ...
        [
            ...
            "&& POSTINCREMENT=0",
            "&& POSTEXEC=0",
            "&& TRANSFER=0 OR SIZE=2 AND GPR RANGE",
        ],
        "SUPPORTED ENCODING",
    )
    canvas.box_text(
        ...
        600,
        [
            "UNSUPPORTED FIELD: CMDERR=NOTSUP",
            "HART NOT HALTED: CMDERR=HALT_RESUME",
            "REG ACCESS ERROR: CMDERR=EXCEPTION",
            "DMACTIVE=0: SILENT ABORT AND FLUSH",
        ],
        "ERROR MAP",
    )
    canvas.box_text(
        ...
        560,
        [
            "SUCCESSFUL READ: DATA0_WE=1",
            "SUCCESSFUL WRITE: NO DATA0 UPDATE",
            "TRANSFER=0: SUCCESSFUL NOOP",
            "COMPLETE LASTS EXACTLY ONE CYCLE",
        ],
    ...
    canvas.box_text(
        ...
        1130,
        [
            "COMMAND PULSE CAPTURES WRITE REGNO DATA0",
            "REQUEST REGS HOLD THROUGH ISSUE BACKPRESSURE",
            "RESPONSE CAPTURES DATA AND MAPS CORE ERROR",
            "HART LOSS IN ISSUE OR WAIT ASSERTS REG FLUSH",
        ],
        "REGISTER UPDATE PRIORITY",
    )
    canvas.box_text(
        ...
        [
            "BUSY = STATE NOT IDLE",
            "COMMAND ERROR VALID ONLY FOR NONZERO CMDERR",
            "DATA0 WRITE ONLY FOR SUCCESSFUL READ",
            "NO COMBINATIONAL REG DATA TO DMI REGS PATH",
        ],
        "OUTPUT GUARANTEE",
    )
    canvas.text(40, 1335, "NOTE: GENERATED BY DOCS/TOOLS/RENDER_STATE_PNGS.PY", scale=2, color=DARK_GRAY)
    canvas.write_png(Path("debug/docs/images/debug_abstract_cmd_fsm.png"))
...
def main() -> None:
    ...
    render_debug_dmi_regs_l3()
    render_debug_halt_ctrl_l3()
    render_debug_reg_access_l3()
    render_debug_abstract_cmd_l3()
