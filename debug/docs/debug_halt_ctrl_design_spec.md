...
![debug_halt_ctrl FSM](images/debug_halt_ctrl_fsm.png)

The L3 PNG is generated by `docs/tools/render_state_pngs.py`.

```text
Legend:
  IF   = external connection
  COMB = combinational logic
  SEQ  = sequential state using clk_i/rst_ni

 IF DM intent ---> COMB transition logic ---> SEQ transaction state
                          ^                         |
 IF core status ----------+                         v
                                            COMB request decode ---> IF core

 IF reset/ack ---> SEQ havereset register ---> IF dmstatus
 IF resume/core -> SEQ resumeack register  ---> IF dmstatus
```

The transaction state, sticky acknowledgement registers, transition logic, and
request decode are shown as separate timing-class blocks.
