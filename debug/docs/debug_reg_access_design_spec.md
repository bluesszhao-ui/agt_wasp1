...
editable source: `debug/docs/diagrams/debug_reg_access_fsm.graffle`
preview export: none
detail level: L3
clock domains: `clk_i/rst_ni` for every `SEQ` block

The editable OmniGraffle diagram separates upstream command/response
interfaces, local handshake and flush logic, command registers, transport FSM,
core GPR interface, and response registers into explicit `IF`, `COMB`, and
`SEQ` timing-class blocks. The historical PNG
`debug/docs/images/debug_reg_access_fsm.png` remains as a reference export.
