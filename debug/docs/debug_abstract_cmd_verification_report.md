...
| Simulation end | `2226ns` |
| Self-check milestones | 218 |
...
| `206ns-346ns` | Read local `misa`, `dcsr`, and core-supplied `dpc`; set and clear `dcsr.step` through Access Register writes | local CSR reads update data0; DCSR writes update only the step bit and do not issue GPR requests | PASS |
| `346ns-546ns` | Transfer-disabled no-op and unsupported type/size/options/registers/CSR writes plus running-hart command | no-op succeeds; unsupported maps to NOTSUP; running maps to HALT_RESUME | PASS |
| `546ns-756ns` | Lose halted state in ISSUE/WAIT, deactivate DM in ISSUE/WAIT, inject busy command, reset active command | flush/error/abort priorities and captured fields remain correct | PASS |
| `756ns-2226ns` | Run 20 deterministic-random valid GPR commands with randomized issue/response delay and error injection | every decoded request and completion matches the reference model | PASS |

## 4. Functional Coverage Summary

| Coverage item | Count | Result |
| --- | ---: | --- |
| Self-check milestones | 218 | PASS |
| Read command classes | 13 | PASS |
| Write command classes | 16 | PASS |
| Local CSR read classes | 5 | PASS |
| Local CSR write classes | 2 | PASS |
...
The RV32 GPR Access Register decoder/controller plus local `misa`/`dcsr`,
core-supplied `dpc`, and `dcsr.step` writes are verified against a mock
`debug_reg_access`. Broader CSR access,
