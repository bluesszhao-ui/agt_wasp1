...
| 131ns-191ns | Cancel halt-pending with resume. | Controller returns running without halted pulse. | PASS |
| 191ns-271ns | Resume while debug busy, then clear busy. | Resume blocked while busy, then running after busy clears. | PASS |
| 271ns-331ns | Single-step from halted. | Running during step, halt-pending after retire, halted after drain. | PASS |
| 331ns-366ns | Assert halt/resume/step together while halted. | Halt priority keeps halted/frozen. | PASS |

## 4. Coverage Summary

```text
tb_core_debug_ctrl coverage: pass=16 halt=5 resume=1 step=1 busy=1 cancel=1 priority=1
