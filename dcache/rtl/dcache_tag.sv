...
// dcache_tag stores valid bits and tags for a direct-mapped data cache.
//
// The block has a combinational lookup path and a clocked refill/update path.
// It owns no data RAM and performs no load/store/refill sequencing; later
// dcache_data and dcache_ctrl modules use this leaf for hit/miss decisions.
module dcache_tag #(
  parameter int LINE_COUNT = 16,
  parameter int LINE_BYTES = 16,
  parameter int ADDR_WIDTH = 32
) (
  input  logic                  clk_i,              // D-cache clock for tag/valid state.
