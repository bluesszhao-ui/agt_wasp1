...
// icache_refill sequences one direct-mapped instruction-cache line refill.
//
// A start request captures the miss address, aligns it to the cache-line base,
// reads one 32-bit word at a time from the downstream memory path, assembles a
// full line, and presents the completed line to the cache tag/data update path.
module icache_refill #(
  parameter int LINE_BYTES = 16,
  parameter int ADDR_WIDTH = 32,
  parameter int DATA_WIDTH = 32
) (
  input  logic                    clk_i,          // I-cache refill controller clock.
  input  logic                    rst_ni,         // Active-low asynchronous reset.
  input  logic                    flush_i,        // Abort any active refill and suppress completion.

  input  logic                    start_valid_i,  // Miss/refill request is valid.
  ...
  mem_req_rsp_if.initiator mem_if                 // Downstream instruction memory/cache path.
);
