...
// tb_icache_tag is a self-checking testbench for the direct-mapped icache tag
...
module tb_icache_tag;
  ...
  initial begin
    ...
    if (pass_count < 130 || lookup_count < 100 || hit_count < 20 ||
      ...
      $fatal(1, "icache_tag coverage goal missed pass=%0d lookup=%0d hit=%0d miss=%0d refill=%0d err=%0d conflict=%0d invalidate=%0d random=%0d",
             pass_count, lookup_count, hit_count, miss_count, refill_count,
             error_count, conflict_count, invalidate_count, random_count);
    end

    $display("tb_icache_tag coverage: pass_count=%0d lookup=%0d hit=%0d miss=%0d refill=%0d err=%0d conflict=%0d invalidate=%0d random=%0d",
             pass_count, lookup_count, hit_count, miss_count, refill_count,
             error_count, conflict_count, invalidate_count, random_count);
    $display("tb_icache_tag PASS");
    $finish;
  end
endmodule
