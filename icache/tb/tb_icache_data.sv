...
// tb_icache_data verifies the direct-mapped I-cache data line storage.
// A reference line array checks refill writes, combinational lookup reads,
// word-offset selection, conflict replacement, and deterministic-random traffic.
module tb_icache_data;
  ...
  function automatic logic [LINE_BITS-1:0] make_line(input logic [31:0] seed);
    ...
    begin
      for (int word = 0; word < WORDS_PER_LINE; word++) begin
        line[word * DATA_WIDTH +: DATA_WIDTH] = seed ^ (32'h1111_0000 + 32'(word));
      end
      make_line = line;
    end
  endfunction
  ...
  task automatic do_refill(
    ...
    begin
      ...
      lookup_addr = addr;
      #1;
      if (lookup_line !== line) begin
        $fatal(1, "%s refill line mismatch index=%0d got=%032x exp=%032x",
               name, index, lookup_line, line);
      end
    end
  endtask
  ...
  initial begin
    ...
    expect_lookup("lookup addr_c", addr_c);
    ...
    #1;
    if (lookup_line !== line_b) begin
      $fatal(1, "write was visible before clock edge");
    end
    @(posedge clk);
    #1;
    refill_valid = 1'b0;
    ...
    refill_count++;
    expect_lookup("write visible after edge", addr_b);
    ...
    for (int i = 0; i < 140; i++) begin
      ...
      rand_line = make_line(lfsr ^ 32'h5A5A_0000);
      if (lfsr[0]) begin
    ...
    if (pass_count < 130 || refill_count < 40 || lookup_count < 70 ||
        word_count < WORDS_PER_LINE || conflict_count < 1 ||
        random_count < 140) begin
      $fatal(1, "icache_data coverage goal missed pass=%0d refill=%0d lookup=%0d word=%0d conflict=%0d random=%0d",
             pass_count, refill_count, lookup_count, word_count,
             conflict_count, random_count);
    end

    $display("tb_icache_data coverage: pass_count=%0d refill=%0d lookup=%0d word=%0d conflict=%0d random=%0d",
             pass_count, refill_count, lookup_count, word_count,
             conflict_count, random_count);
    $display("tb_icache_data PASS");
    $finish;
  end
endmodule
