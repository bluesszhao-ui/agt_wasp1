...
// tb_icache_refill verifies the I-cache line refill sequencer.
// It models the downstream memory path, checks every issued word request, and
// compares completed line data/error outputs against a reference line builder.
module tb_icache_refill;
  ...
  function automatic logic [DATA_WIDTH-1:0] word_for_addr(input logic [ADDR_WIDTH-1:0] addr);
    word_for_addr = 32'h9000_0000 ^ addr;
  endfunction
  ...
  initial begin
    ...
    if (pass_count < 200 || start_count < 20 || req_count < 80 ||
      ...
      $fatal(1, "icache_refill coverage goal missed pass=%0d start=%0d req=%0d rsp=%0d done=%0d err=%0d flush=%0d bp=%0d random=%0d",
             pass_count, start_count, req_count, rsp_count, done_count,
             error_count, flush_count, backpressure_count, random_count);
    end

    $display("tb_icache_refill coverage: pass_count=%0d start=%0d req=%0d rsp=%0d done=%0d err=%0d flush=%0d backpressure=%0d random=%0d",
             pass_count, start_count, req_count, rsp_count, done_count,
             error_count, flush_count, backpressure_count, random_count);
    $display("tb_icache_refill PASS");
    $finish;
  end
endmodule
